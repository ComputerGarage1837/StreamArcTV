...
public partial class PlayerPage : AppPage
{
    ...
    public PlayerPage(string url, string title, bool live, string? watchKey = null)
    {
        ...
        BtnNextPlay.Click += NextPlayClick;
        ...
        Seek.AddHandler(PointerPressedEvent, (_, _) => _seekDragging = true, RoutingStrategies.Tunnel);
        Seek.AddHandler(PointerReleasedEvent, (_, _) => { _seekDragging = false; SeekTo((long)(Seek.Value / 1000.0 * Math.Max(1, _lengthMs))); }, RoutingStrategies.Tunnel);
        Seek.AddHandler(KeyDownEvent, (_, e) =>
        {
            if (e.Key == Key.Left) { SeekBy(-SEEK_STEP_MS); e.Handled = true; }
            else if (e.Key == Key.Right) { SeekBy(SEEK_STEP_MS); e.Handled = true; }
        }, RoutingStrategies.Tunnel);

        Overlay.PointerMoved += (_, _) => ShowControls();
        Overlay.PointerPressed += (_, e) =>
        {
            if (!e.GetCurrentPoint(Overlay).Properties.IsLeftButtonPressed) return;
            if (e.ClickCount == 2) App.Window?.SetFullScreen(!App.Window.IsFullScreen);
            else if (!_controlsVisible) ShowControls();
            else if (ReferenceEquals(e.Source, Overlay)) TogglePause();
        };
        Overlay.AddHandler(KeyDownEvent, OnOverlayKey, RoutingStrategies.Tunnel);
    ...
    /// Toasts go over the video, which covers the window's own toast.
    public override Panel? ToastHost => Overlay;
    ...
    public override void OnPause()
    {
        ...
        // The overlay is a window of its own; hide it with the page.
    ...
    private void OnOverlayKey(object? sender, KeyEventArgs e)
    {
        ...
        if (e.Key == Key.F11 || (e.Key == Key.F && e.KeyModifiers.HasFlag(KeyModifiers.Meta) && e.KeyModifiers.HasFlag(KeyModifiers.Control))) { App.Window?.SetFullScreen(!App.Window.IsFullScreen); e.Handled = true; return; }
    ...
    private bool HandleKey(KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Space: if (FocusedElement() is not Button) { TogglePause(); return true; } return false;
            case Key.F: if (e.KeyModifiers == KeyModifiers.None) { App.Window?.SetFullScreen(!App.Window.IsFullScreen); return true; } break;
    ...
    private IInputElement? FocusedElement() => TopLevel.GetTopLevel(Overlay)?.FocusManager?.GetFocusedElement() ?? Ui.Focused;
    ...
    {
        if (NextBox.IsVisible || ErrorBox.IsVisible) return;
        SetControlsVisible(false);
        var f = FocusedElement();
        if (f is Control { IsEffectivelyVisible: false } || f is Button or Slider) Overlay.Focus();
    }

    private void SetControlsVisible(bool on)
    {
        _controlsVisible = on;
        TitleBar.IsVisible = on;
        Controller.IsVisible = on;
        TxtLiveStatus.IsVisible = _isLive && on;
        BtnSeekBack.IsVisible = on;
        BtnSeekFwd.IsVisible = on;
        Overlay.Cursor = on ? Ui.ArrowCursor : Ui.NoCursor;
    }
    ...
    private void Watchdog(MediaPlayer p)
    {
        ...
        if (_userPaused || _ended || ErrorBox.IsVisible) return;
    ...
    {
        var p = _player;
        if (p == null) { if (!ErrorBox.IsVisible) InitPlayer(); return; }
    ...
    private void SetPlayIcon(bool playing) => IconPlayPause.Data = Ui.Icon(playing ? "IconPause" : "IconPlay");
    ...
    private void UpdateStatus()
    {
        ...
        var isBuffering = (_buffering || (!p.IsPlaying && !_userPaused && !_ended)) && !ErrorBox.IsVisible;
        ...
        else if (!_pendingRetry) BufferBox.IsVisible = false;

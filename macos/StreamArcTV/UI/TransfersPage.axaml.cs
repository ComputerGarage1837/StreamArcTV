...
public partial class TransfersPage : AppPage
{
    ...
    private class Row
    {
        ...
        public ThinProgress Progress = null!;
    ...
    private void Refresh(bool keepScroll = false)
    {
        ...
        TxtEmpty.Text = _type == TransferType.RECORDING
            ? "No recordings yet. Hold Enter on a channel in the TV guide and choose Record."
            : "No downloads yet. Hold Enter on a movie or episode and choose Download.";
        TxtEmpty.IsVisible = items.Count == 0;
        BtnDeleteAll.IsVisible = items.Count != 0;
    }
    ...
    private Row Build(TransferJob j)
    {
        ...
        r.Title = new TextBlock { FontSize = 15, FontWeight = FontWeight.SemiBold, Foreground = Ui.Brush("TextBrush"), TextTrimming = TextTrimming.CharacterEllipsis };
        ...
        r.Progress = new ThinProgress { Margin = new Thickness(0, 4, 0, 0), IsVisible = false };
        text.Children.Add(r.Title); text.Children.Add(r.Subtitle); text.Children.Add(r.Status); text.Children.Add(r.Progress);
        grid.Children.Add(text);
        r.Play = new Button { Content = "Play", Classes = { "small" }, Margin = new Thickness(8, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center };
        r.Delete = new Button { Content = "Delete", Classes = { "small" }, Margin = new Thickness(6, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center, Foreground = Ui.Brush("DangerBrush") };
        Grid.SetColumn(r.Play, 1); Grid.SetColumn(r.Delete, 2);
        grid.Children.Add(r.Play); grid.Children.Add(r.Delete);
        r.Card = new FocusCard { Content = grid, Classes = { "stream" }, Margin = new Thickness(0, 0, 0, 8), Padding = new Thickness(14, 10, 8, 10) };
    ...
    {
        if (_type != TransferType.RECORDING) { NoticeBox.IsVisible = false; return; }
        ...
        if (running.Count == 0 && scheduled.Count == 0) { NoticeBox.IsVisible = false; return; }

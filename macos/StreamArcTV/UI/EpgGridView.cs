...
/// </summary>
public class EpgGridView : Control
{
    ...
    private readonly Dictionary<string, Bitmap?> _logos = new();
    ...
    private static readonly IBrush BgBrush = new ImmutableSolidColorBrush(0xFF0A1628);
    private static readonly IBrush ColBrush = new ImmutableSolidColorBrush(0xFF0D1F3C);
    private static readonly IBrush HeaderBrush = new ImmutableSolidColorBrush(0xFF0F2140);
    private static readonly IPen LinePen = new ImmutablePen(new ImmutableSolidColorBrush(0xFF1E3A5F), 1);
    private static readonly IPen NowPen = new ImmutablePen(new ImmutableSolidColorBrush(0xFFA78BFA), 2);
    private static readonly IBrush CellBrush = new ImmutableSolidColorBrush(0xFF2B3243);
    private static readonly IBrush CellNowBrush = new ImmutableSolidColorBrush(0xFF3A4358);
    private static readonly IBrush CellPastBrush = new ImmutableSolidColorBrush(0xFF222838);
    private static readonly IBrush CellFocusBrush = new ImmutableSolidColorBrush(0xFFF8FAFC);
    private static readonly IPen FocusStroke = new ImmutablePen(new ImmutableSolidColorBrush(0xFFA78BFA), 2.5);
    private static readonly IBrush RowFocusBrush = new ImmutableSolidColorBrush(0x1AFFFFFF);
    private static readonly IBrush TitleBrush = Brushes.White;
    private static readonly IBrush TitleFocusBrush = new ImmutableSolidColorBrush(0xFF0A1628);
    private static readonly IBrush SubBrush = new ImmutableSolidColorBrush(0xFF94A3B8);
    private static readonly IBrush SubFocusBrush = new ImmutableSolidColorBrush(0xFF475569);
    private static readonly IBrush MarkerBrush = new ImmutableSolidColorBrush(0xFFA78BFA);
    private static readonly IBrush TimeBrush = new ImmutableSolidColorBrush(0xFFCBD5E1);
    ...
    private static IImage Placeholder => ImageLoader.Placeholder;
    ...
    public void SetChannels(List<Data.Stream> list)
    {
        ...
        _scrollY = 0;
        InvalidateVisual();
        if (list.Count > 0) Dispatcher.UIThread.Post(NotifyFocus);
    }
    ...
    public void SetEpg(string streamId, List<EpgProgramme> programmes)
    {
        ...
        if (!_invalidatePending)
        {
            ...
            DispatcherTimer.RunOnce(() => { _invalidatePending = false; InvalidateVisual(); }, TimeSpan.FromMilliseconds(80));
        }
    ...
    private double MaxScrollX() => Math.Max(0, (_windowEnd - _windowStart) / 60.0 * _pxPerMin - (Bounds.Width - _channelColW));
    private double MaxScrollY() => Math.Max(0, _channels.Count * RowH - (Bounds.Height - HeaderH));
    ...
    private void EnsureFocusVisible()
    {
        ...
        var viewW = Bounds.Width - _channelColW;
        ...
        var viewH = Bounds.Height - HeaderH;
    ...
    private static FormattedText Text(string s, double size, IBrush brush, double maxWidth, FontWeight? weight = null)
    {
        var ft = new FormattedText(s, CultureInfo.CurrentUICulture, FlowDirection.LeftToRight, Face, size, brush)
        {
    ...
    public override void Render(DrawingContext c)
    {
        ...
        c.FillRectangle(BgBrush, new Rect(0, 0, width, height));
        var now = Format.NowSec;
        ...
        // Programme area
        using (c.PushClip(new Rect(_channelColW, HeaderH, Math.Max(0, width - _channelColW), Math.Max(0, height - HeaderH))))
        {
            for (var row = firstRow; row <= lastRow; row++)
            {
                ...
                if (rowFocused) c.FillRectangle(RowFocusBrush, new Rect(_channelColW, top, width - _channelColW, RowH));
                var list = ProgrammesFor(ch);
                ...
                foreach (var p in blocks)
                {
                    ...
                    var brush = isFocusedCell ? CellFocusBrush
                        ...
                        : CellBrush;
                    c.DrawRectangle(brush, isFocusedCell ? FocusStroke : null, rect, Corner, Corner);
        ...
        c.FillRectangle(HeaderBrush, new Rect(0, 0, width, HeaderH));
        ...
        using (c.PushClip(new Rect(_channelColW, 0, Math.Max(0, width - _channelColW), HeaderH)))
        {
            var t = _windowStart;
            while (t <= _windowEnd)
            {
            ...
            if (now >= _windowStart && now <= _windowEnd)
            {
                ...
                {
                    g.BeginFigure(new Point(nx - 7, HeaderH - 9), true);
                    g.LineTo(new Point(nx + 7, HeaderH - 9));
                    g.LineTo(new Point(nx, HeaderH));
                    g.EndFigure(true);
                }
        ...
        // Channel column
        c.FillRectangle(ColBrush, new Rect(0, 0, _channelColW, height));
        using (c.PushClip(new Rect(0, HeaderH, _channelColW, Math.Max(0, height - HeaderH))))
        {
            ...
            for (var row = firstRow; row <= lastRow; row++)
            {
                var ch = _channels[row];
                var top = HeaderH + row * RowH - _scrollY;
                if (row == _focusRow && focused) c.FillRectangle(RowFocusBrush, new Rect(0, top, _channelColW, RowH));
                ...
                if (bmp != null)
                {
                    var scale = Math.Min(logoW / bmp.PixelSize.Width, logoH / bmp.PixelSize.Height);
                    var w = bmp.PixelSize.Width * scale; var h = bmp.PixelSize.Height * scale;
    ...
    private Bitmap? LogoFor(Data.Stream ch)
    {
        var id = ch.StreamId;
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Up:
                ...
                _focusRow--; Moved(); e.Handled = true; return;
            case Key.PageDown:
                _focusRow = Math.Min(_channels.Count - 1, _focusRow + Math.Max(1, (int)((Bounds.Height - HeaderH) / RowH) - 1)); Moved(); e.Handled = true; return;
            case Key.PageUp:
                _focusRow = Math.Max(0, _focusRow - Math.Max(1, (int)((Bounds.Height - HeaderH) / RowH) - 1)); Moved(); e.Handled = true; return;
            case Key.Home:
    ...
    protected override void OnPointerWheelChanged(PointerWheelEventArgs e)
    {
        ...
        // A trackpad gives both axes; a mouse wheel scrolls rows, Shift+wheel the timeline.
        var dx = e.Delta.X != 0 ? -e.Delta.X : (e.KeyModifiers.HasFlag(KeyModifiers.Shift) ? -e.Delta.Y : 0);
        var dy = e.Delta.X != 0 || e.KeyModifiers.HasFlag(KeyModifiers.Shift) ? 0 : -e.Delta.Y;
        _scrollX = Math.Clamp(_scrollX + dx * 60, 0, MaxScrollX());
        _scrollY = Math.Clamp(_scrollY + dy * 60, 0, MaxScrollY());
        InvalidateVisual();
        e.Handled = true;
    }

...
public partial class MultiViewPage : AppPage
{
    ...
    public override void OnResume() { if (!_dead) { foreach (var t in _tiles) t.View.IsVisible = true; StartAll(); } }
    public override void OnPause() { StopAll(); foreach (var t in _tiles) t.View.IsVisible = false; }
    ...
    private void BuildGrid()
    {
        ...
        for (var i = 0; i < count; i++)
        {
            var n = i;
            var tile = new Tile();
            var overlay = new Panel { Background = Brushes.Transparent, HorizontalAlignment = HorizontalAlignment.Stretch, VerticalAlignment = VerticalAlignment.Stretch };
            ...
            tile.Name = new TextBlock { FontSize = 12, Foreground = Ui.Brush("TextBrush"), IsVisible = false };
            var nameBox = new Border { Background = Ui.Color(0x99000000), Padding = new Thickness(8, 3, 8, 3), HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Bottom, Margin = new Thickness(8), Child = tile.Name };
            overlay.Children.Add(tile.Hint);
            overlay.Children.Add(nameBox);
            tile.View = new VideoView { Content = overlay };
            tile.Card = new FocusCard { Content = tile.View, Classes = { "tile" } };
            tile.Card.Click += () => { Select(n); PickChannel(n); };
            tile.Card.LongClick += () => { Select(n); TileMenu(n); };
            tile.Card.GotFocus += (_, _) => Select(n);
            _tiles.Add(tile);
    ...
    private void PickChannel(int i)
    {
        ...
        var filter = new TextBox { Watermark = "Search channels…" };
        var list = new ListBox { Height = 320, Margin = new Thickness(0, 8, 0, 0), FontSize = 14 };
        var shown = _channels;
        ...
        filter.KeyDown += (_, e) => { if (e.Key == Key.Down) { list.Focus(); if (list.ItemCount > 0 && list.SelectedIndex < 0) list.SelectedIndex = 0; e.Handled = true; } };
    ...
    private void Assign(int i, Data.Stream stream, bool save)
    {
        ...
        var tile = _tiles[i];
        tile.Stream = stream;
        tile.Hint.IsVisible = false;
        tile.Name.IsVisible = true;
        if (save)
        {
            var slots = _prefs.MultiviewSlots;
    ...
    private void Remove(int i)
    {
        ...
        tile.Hint.IsVisible = true;
        tile.Name.IsVisible = false;
        var slots = _prefs.MultiviewSlots;
    ...
    private void Play(Tile tile)
    {
        ...
        catch (Exception e) { tile.Hint.Text = e.Message; tile.Hint.IsVisible = true; return; }
        try
        {
            ...
            p.Playing += (_, _) => Ui.Post(() => tile.Hint.IsVisible = false);
        ...
        catch (Exception e)
        {
            ...
            tile.Hint.Text = e.Message; tile.Hint.IsVisible = true;
        }
    }

...
/// downloaded once into the app cache, decoded at a modest size on a worker thread, frozen, and
/// shown when they arrive if the target still wants the same URL.
/// </summary>
public static class ImageLoader
{
    ...
    private static readonly Dictionary<string, Task<BitmapSource?>> InFlight = new();
    ...
    public static ImageSource Placeholder => Ui.Res<ImageSource>("PlaceholderImage");
    ...
    public static void Load(Image target, string? url, int decodeWidth = 480, bool placeholder = true)
    {
        ...
        if (placeholder) target.Source = Placeholder; else target.Source = null;
        Get(url, decodeWidth).ContinueWith(t =>
        {
            var bmp = t.Status == TaskStatus.RanToCompletion ? t.Result : null;
            if (bmp == null) return;
            target.Dispatcher.BeginInvoke(() => { if (Equals(target.Tag, url)) target.Source = bmp; });
        });
    }

    /// Loads a bitmap and calls back on the UI thread (used by the drawn guide grid).
    public static void Get(string url, int decodeWidth, Action<BitmapSource?> onDone)
    {
        Get(url, decodeWidth).ContinueWith(t =>
        {
            var bmp = t.Status == TaskStatus.RanToCompletion ? t.Result : null;
            Application.Current?.Dispatcher.BeginInvoke(() => onDone(bmp));
        });
    }

    public static Task<BitmapSource?> Get(string url, int decodeWidth)
    {
        lock (Memory) if (Memory.TryGetValue(url, out var cached)) return Task.FromResult<BitmapSource?>(cached);
        lock (InFlight)
        {
    ...
    private static async Task<BitmapSource?> Fetch(string url, int decodeWidth)
    {

...
public static class Folders
{
    ...
    {
        var baseDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyVideos), "Stream Arc TV");
        if (string.IsNullOrWhiteSpace(Environment.GetFolderPath(Environment.SpecialFolder.MyVideos)))
            baseDir = Path.Combine(AppPaths.Data, "Media");
    ...
    {
        if (folder == null) return "Videos / Stream Arc TV / " + (type == TransferType.RECORDING ? "Recordings" : "Downloads");
        try
        {
    ...
    private static readonly Regex Bad = new("[\\\\/:*?\"<>|\\x00-\\x1F]", RegexOptions.Compiled);

package com.chocolaterabbit.app.update
...
import com.chocolaterabbit.app.data.Prefs
import com.chocolaterabbit.app.net.Http
...
/**
 * Checks the repository's `release/update-chocolate-rabbit.json` feed (see
 * UPDATE_FORMAT.md at the repository root) for a newer build and offers to
 * download and install it, with a "skip this version" option.
 *
 * Same mechanism as Stream Arc TV: the feed is the source of truth. It names
 * the APK asset attached to the matching GitHub release
 * (`chocolate-rabbit-v<versionName>`) and carries its SHA-256 and size so the
 * download can be verified before it is handed to the installer. The release
...
object UpdateChecker {
    ...
    const val FEED_PATH = "release/update-chocolate-rabbit.json"
    const val TAG_PREFIX = "chocolate-rabbit-v"
    ...
    private val feedApiUrl: String
        get() = "https://api.github.com/repos/$repo/contents/$FEED_PATH?ref=main"

    private val feedRawUrl: String
        get() = "https://raw.githubusercontent.com/$repo/main/$FEED_PATH"

    /**
     * @param manual true when the user chose "Check for updates": always
    ...
    fun fetchLatest(): Release {
        ...
        val tag = "$TAG_PREFIX$versionName"

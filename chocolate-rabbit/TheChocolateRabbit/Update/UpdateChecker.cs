...
namespace TheChocolateRabbit.Update;

/// <summary>
/// Checks the repository's `release/update-chocolate-rabbit.json` feed (see UPDATE_FORMAT.md at
/// the repository root) for a newer build and offers to download and install it, with a "skip
/// this version" option. Same mechanism as Stream Arc TV for Windows.
///
/// The feed is the source of truth: it names the setup program and zip attached to the matching
/// GitHub release (`chocolate-rabbit-v&lt;versionName&gt;`) and carries their SHA-256 and size so the
/// download can be verified before it is installed. The release notes are fetched from the GitHub
/// release itself, best-effort.
/// </summary>
public static class UpdateChecker
{
    ...
    public const string FEED_FILE = "release/update-chocolate-rabbit.json";
    public const string TAG_PREFIX = "chocolate-rabbit-v";
    ...
    public static string TagFor(string versionName) => $"{TAG_PREFIX}{versionName}";

    /// <param name="manual">true when the user chose "Check for updates": always reports the outcome and ignores the "skipped" version.</param>
    ...
    public static Release FetchLatest()
    {
        ...
        using (feedDoc)
        {
            ...
            var setup = ReadPackage(feed.Get("setup"), tag);
...
/// Small System.Text.Json helpers.

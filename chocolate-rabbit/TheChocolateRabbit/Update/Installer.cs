...
/// then installs it. An installed copy (set up by The-Chocolate-Rabbit-Setup-*.exe) is updated by
/// running the new setup program silently; a portable copy (unpacked from the zip) has its files
/// swapped in place.
/// </summary>
public static class Installer
{
    ...
    /// restarts. Leftover "*.old" files are removed on the next start. If the folder needs
    /// administrator rights, an elevated helper does the swap.
    public static async void Install(string zipFile)
    {
        ...
        {
            Dialogs.Alert("Extract the app first", $"{BuildInfo.AppName} is running from a temporary folder (the zip was opened without extracting it). Extract the zip into a folder of your own, run TheChocolateRabbit.exe from there, and update again.");
            return;
        }

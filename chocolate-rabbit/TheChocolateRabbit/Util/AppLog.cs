...
namespace TheChocolateRabbit.Util;

/// <summary>
/// App-wide diagnostic log, appended to a file under the app's data folder. Sign-up details are
/// never written here.
/// </summary>
public static class AppLog
{
    ...
    private static string OldFile => Path.Combine(Dir, "chocolate-rabbit.1.log");

    public static void Init()
    {
        Directory.CreateDirectory(Dir);
        _file = Path.Combine(Dir, "chocolate-rabbit.log");
        I("App", $"{BuildInfo.AppName} {BuildInfo.VersionName} · Windows {Environment.OSVersion.Version} · " +
    ...
    /// Writes the log to a text file the user chooses.
    public static void Share(System.Windows.Window owner)
    {
        var dlg = new Microsoft.Win32.SaveFileDialog
        {
            Title = "Export log",
            FileName = $"chocolate-rabbit-log-{BuildInfo.VersionName}.txt",
...
public static class AppPaths
{
    public static string Data { get; } = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "TheChocolateRabbit");

    public static void Ensure() => Directory.CreateDirectory(Data);
}

...
public static class TransferService
{
    ...
    public static void RescheduleAll()
    {
        ...
        foreach (var job in Store.All())
        {
            ...
            if (job.EndAt <= now) { Store.Update(job.Id, j => { j.State = TransferState.FAILED; j.Error = "Missed"; }); UnscheduleTask(job.Id); }
    ...
    private static async Task Run(TransferJob job, CancellationToken ct)
    {
        ...
        if (job.Type == TransferType.RECORDING) UnscheduleTask(id);
    ...
    /// True when the only way out is a dial-up / mobile-broadband (PPP) interface. A phone shared
    /// over USB or Wi-Fi looks like Ethernet or Wi-Fi to a Mac, so this is rarely true here.

...
public partial class SeriesPage : AppPage
{
    ...
    private readonly List<(FocusCard Row, Episode Ep, TextBlock Title, ThinProgress Progress)> _rows = new();
    ...
    private async void Load()
    {
        ...
        try
        {
            ...
            if (episodes.Count == 0) { TxtError.Text = "No episodes found."; TxtError.IsVisible = true; }
            else
            {
                _episodes = episodes;
                Render();
                BtnDownloadAll.IsVisible = true;
                if (IsResumed) Dispatcher.UIThread.Post(() => _rows.FirstOrDefault().Row?.Focus(), DispatcherPriority.Input);
            }
        }
        catch (Exception e)
        {
    ...
    private void Render()
    {
        ...
        for (var i = 0; i < _episodes.Count; i++)
        {
            ...
            if (firstOfSeason)
            {
                var head = new Grid { Margin = new Thickness(6, i == 0 ? 0 : 10, 0, 6), ColumnDefinitions = new ColumnDefinitions("*,Auto") };
                head.Children.Add(new TextBlock { Text = $"SEASON {ep.Season}", Classes = { "section" }, Margin = new Thickness(0), VerticalAlignment = VerticalAlignment.Center });
                var season = ep.Season;
                var dl = new Button { Content = "Download…", Classes = { "small" } };
            ...
            var infoText = new TextBlock { Text = info, FontSize = 12, Foreground = Ui.Brush("TextMutedBrush"), TextWrapping = TextWrapping.Wrap, TextTrimming = TextTrimming.CharacterEllipsis, MaxLines = 2, Margin = new Thickness(0, 2, 0, 0), IsVisible = !string.IsNullOrWhiteSpace(info) };
            var progress = new ThinProgress { Margin = new Thickness(0, 6, 0, 0), IsVisible = false };
            stack.Children.Add(title); stack.Children.Add(infoText); stack.Children.Add(progress);
            var row = new FocusCard { Content = stack, Classes = { "stream" }, Margin = new Thickness(0, 0, 0, 6), Padding = new Thickness(14, 10, 14, 10) };
            row.Click += () => Play(ep);
    ...
    private void Play(Episode ep)
    {
        ...
        catch (Exception e) { TxtError.Text = e.Message; TxtError.IsVisible = true; return; }
